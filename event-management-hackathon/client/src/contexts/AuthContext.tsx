...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    ...
    const text = await res.text();
    let data = null;
    try { data = text ? JSON.parse(text) : null; } catch (e) { console.error('Failed to parse login response JSON', e); }
  ...
  const register = async (userData: { name?: string; email: string; password: string; college?: string; role?: AuthRole }) => {
    ...
    const text = await res.text();
    let data = null;
    try { data = text ? JSON.parse(text) : null; } catch (e) { console.error('Failed to parse registration response JSON', e); }
