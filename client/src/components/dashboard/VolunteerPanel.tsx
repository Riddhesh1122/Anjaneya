  ...
  return (
    <div className={`rounded-xl border ${surface} overflow-hidden`}>
      ...
      <div className={`px-5 py-4 border-b ${divider} flex items-center justify-between`}>
        <div>
          <h3 className={`text-sm font-semibold ${textPrimary}`}>Top Volunteers</h3>
          <p className={`text-xs mt-0.5 ${textMuted}`}>AI match score & performance</p>
        </div>
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="relative flex-shrink-0">
                ...
                <div className={`absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-black text-white ${
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <p className={`text-xs font-semibold truncate ${textPrimary}`}>{vol.name}</p>
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${statusStyle[vol.status]}`}>
                    {vol.status}
                  </span>
                </div>
                <p className={`text-[11px] truncate ${textMuted}`}>{vol.role}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[10px] font-semibold ${textMuted} w-8 text-right`}>{vol.score}%</span>
            </div>

            {/* Skills */}
            <div className="flex gap-1 mt-1.5">
              {vol.skills.map(s => (
                <span key={s} className={`text-[10px] px-1.5 py-0.5 rounded ${tagBg}`}>{s}</span>
              ))}
            </div>
          </motion.div>
        ))}
